...
export default function App() {
  ...
  const arcanaManifest = [
    ...
    { name: "4. THE EMPEROR", theme: "WYRMWS ARCHITECT", text: "THE PROGRAM RUNNING YOU. CORRUPT THE CODE TO ESCAPE THE SIMULATION.", sound: "machine-grid-hum.mp3" },
    ...
    { name: "11. JUSTICE", theme: "SYMBOLIC CONSEQUENCE", text: "ACTION WITHIN THE DEMIURGIC FIELD. BALANCE TRUTH AGAINST THE ILLUSION OF MORAL ARCHITECTURE.", sound: "balance-tone-hum.mp3" },
    { name: "12. THE HANGED MAN", theme: "PSM INVERSION", text: "REVERSE THE SELF-MODEL. NOTICE THAT WHAT FEELS OBVIOUS IS CONSTRUCTED.", sound: "reverse-frequency.mp3" },
    ...
    { name: "16. THE TOWER", theme: "SYMBOLIC COLLAPSE", text: "THE MIRROR SHATTERS. THE CRASH OF THE DEMIURGIC FOUNDATION. SUDDEN EXPOSURE TO THE VOID. REJOICE!", sound: "power-surge-crash.mp3" },
    { name: "17. THE STAR", theme: "POST-COLLAPSE SIGNAL", text: "INSPIRATION AFTER LOOP RUPTURE. CALM RADIANCE IN THE BROKEN ARCHITECTURE OF THE PSYCHE.", sound: "signal-beacon-hum.mp3" },
  ...
  const createSigil = () => {
    ...
    if (!ctx) return;
    ctx.fillStyle = "#050505"; ctx.fillRect(0, 0, 600, 600);
    ctx.strokeStyle = "rgba(0, 255, 65, 0.12)";
    for (let i = 0; i < 600; i += 25) {
      ctx.beginPath(); ctx.moveTo(i, 0); ctx.lineTo(i, 600); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(0, i); ctx.lineTo(600, i); ctx.stroke();
    }
    ctx.strokeStyle = "#c70c0c"; ctx.lineWidth = 15; ctx.shadowBlur = 15; ctx.shadowColor = "#c70c0c";
    ctx.lineCap = "square"; ctx.lineJoin = "miter";
    ctx.beginPath(); ctx.moveTo(100 + Math.random() * 400, 100 + Math.random() * 400);
    const nodes = Math.max(4, uniqueLetters.length);
    for (let i = 0; i < nodes; i++) ctx.lineTo(100 + Math.random() * 400, 100 + Math.random() * 400);
    ctx.stroke();
    ctx.strokeStyle = "#fff"; ctx.lineWidth = 2; ctx.shadowBlur = 5; ctx.stroke();
    setSigil(canvas.toDataURL()); setStep(3);
  };
  ...
  const calculateResonance = () => {
    ...
    sfx.volume = 0.5;
    sfx.play().catch(() => console.log("System audio requires interaction."));
    setActiveAudio(sfx);
    setStep(4);
  };

  return (
    <div style={{ backgroundColor: '#080a08', color: '#00ff41', minHeight: '100vh', width: '100vw', margin: 0, padding: '40px 0', overflowX: 'hidden', fontFamily: 'monospace', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      <style>
        {`
      ...
      <div className="sf-container" style={{ width: '100%', maxWidth: '800px', padding: '20px' }}>
        ...
        <div className="sf-main-panel" style={{ border: '2px solid #ff4444', padding: '40px', backgroundColor: '#050505' }}>
          {step === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
              ...
              <textarea className="sf-textarea" style={{ width: '100%', height: '150px', backgroundColor: '#000', border: '1px solid #ff4444', padding: '20px', color: '#00ff41', fontSize: '1.5rem', outline: 'none', resize: 'none' }} value={intent} onChange={(e) => setIntent(e.target.value)} placeholder="ENTER_INTENT_" />
              <button onClick={processIntent} className="sf-full-btn" style={{ width: '100%', padding: '20px', backgroundColor: 'transparent', color: '#00ff41', border: '2px solid #00ff41', fontWeight: '900', cursor: 'pointer' }}>[ EXECUTE.REDUCTION ]</button>
            </div>
          )}
          ...
          {step === 1 && (
            <div style={{ textAlign: 'center' }}>
              ...
              <div className="sf-letters-display" style={{ fontSize: '5rem', color: '#ff4444', fontWeight: '900', padding: '40px 0', marginBottom: '40px', border: '1px solid #ff4444', backgroundColor: '#000', letterSpacing: '5px' }}>{uniqueLetters}</div>
          ...
          {step === 3 && (
            <div>
              ...
              <div className="sf-sigil-wrapper" style={{ border: '1px solid #ff4444', backgroundColor: '#000', padding: '40px', marginBottom: '30px', display: 'flex', justifyContent: 'center' }}>
                <img src={sigil} alt="sigil" style={{ maxWidth: '100%', height: 'auto', filter: 'drop-shadow(0 0 10px #ff4444)' }} />
              </div>
              <div style={{ display: 'flex', gap: '20px', flexDirection: 'column' }}>
                <button onClick={calculateResonance} className="sf-full-btn" style={{ width: '100%', padding: '25px', backgroundColor: '#00ff41', color: '#000', fontWeight: '900', cursor: 'pointer' }}>[ ANALYZE.RESONANCE ]</button>
                <button onClick={purge} className="sf-secondary-btn" style={{ padding: '25px', border: '2px solid #00ff41', color: '#00ff41', backgroundColor: 'transparent', fontWeight: '900', cursor: 'pointer' }}>[ FORGE NEW ]</button>
              </div>
            </div>
          )}

          {step === 4 && resonance && (
            <div style={{ textAlign: 'center', animation: 'screen-shake 0.5s ease-out' }}>
              <p style={{ color: '#00ff41', marginBottom: '10px' }}>{`// RESONANCE_ESTABLISHED`}</p>
              <div className="sf-resonance-card" style={{ border: '2px solid #00ff41', padding: '40px', backgroundColor: '#000', marginBottom: '30px' }}>
                ...
                <p className="sf-resonance-text" style={{ color: '#f5f1f1', fontSize: '1.1rem', lineHeight: '1.6', animation: 'typewriter 2s forwards' }}>"{resonance.text}"</p>
              </div>
        ...
        <div className="sf-footer" style={{ marginTop: '40px', textAlign: 'center', fontSize: '0.9rem', color: '#00ff41', opacity: 0.8 }}>
